...
- **Editing past days** — `CalendarScreen` → `DaySummaryCard` let a parent/kid retroactively edit a prior date's log via `logFoodForDay`/`removeFoodForDay`/`toggleGoalForDay`, which mutate `dayLogs[dateStr]` directly (today's edits use the separate `logFood`/`incrementGoal` actions that mutate the live `today*` fields instead).
